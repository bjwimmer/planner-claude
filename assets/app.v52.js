...
function initThreadRegistry(){
  ...
  function renderThreads(){
    ...
    threadsEl.innerHTML = activeThreads.length ? activeThreads.map(t=>{
      ...
      const backlinksHtml = links.length ? `<div class="meta" style="margin-top:6px; flex-wrap:wrap">
        ${links.map(l=>`<a href="strategic-life-map.html?focusGoal=${encodeURIComponent(l.goalId)}" class="small" style="margin-right:10px">Linked: ${escapeHtml(l.domain)} → ${escapeHtml(l.goalTitle)} (${escapeHtml(l.hLabel)})</a>`).join("")}
      </div>` : "";
